...
| Which local MCP tool did Claude or Codex actually call? | eBPF anonymous-IPC lineage plus bounded stdio JSON-RPC admission and the five-action MCP semantic graph. | [MCP stdio observation](designs/mcp-stdio-observation/README.zh.md), `sudo -E python3 tests/v2/regression/test_all.py --case probe_claude_mcp`, or `--case probe_codex_mcp` |
