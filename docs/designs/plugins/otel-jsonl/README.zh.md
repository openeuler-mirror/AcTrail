# OTEL JSON exporter 插件设计文档

本目录收录 AcTrail 内置 `otel-jsonl` observation consumer 的设计文档。该插件在
trace 运行期间消费 semantic action，将选中的 action 编码为 OTLP JSON，再通过
配置选中的 exporter 异步交付。当前支持本地 JSONL 文件和 JSON-RPC 2.0 over
HTTP(S)，后续 exporter 继续作为独立实现扩展。
...
2. [Exporter 选择协议](exporter-selection.zh.md)
   - 定义 `file` 与 `json_rpc_http` 的分层配置和扩展边界。
   - 规定 JSON-RPC request/response、超时、重试和失败隔离语义。
   - 说明 Web 条件表单与运行时配置生命周期。

3. [Semantic action kind 选择策略设计](action-kind-selection.zh.md)
   - 定义 `action_kinds` 布尔映射及 `default` 语义。
   - 规定 consumer queue 前过滤、配置校验和上游 TTY 保护约束。
   - 明确插件未加载时不要求配置，本次不修改 `actrailviewer`。

4. [目标文件路径](targeted-file-paths.zh.md)
   - 索引整个插件从描述包、发现、加载到 exporter 交付的关键路径。
...
| `exporter-selection.zh.md` | exporter 配置与交付协议 | exporter 类型、字段、交付或失败语义变化时 |
