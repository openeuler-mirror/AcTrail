...
WIT package 中的 world 是 `control-plugin`。运行时要求 component 导出以下 interface 和函数：
...
AcTrail 不应该把所有行为都无条件送进插件。命令和网络控制先由本地显式策略命中插件实例；文件访问控制先由 fanotify 和黑白灰名单快路径筛选，只有需要插件决策的灰名单请求才进入插件。
...
AcTrail 当前只接受这些短 token 和 query 名称，不保留旧长 token 兼容。
...
`file-policy-read(context-ref: string, query: string)` 当前只接受 `context-ref = "f"` 和 `query = "matched-rule.v1"`，返回 `file-policy-view`：
...
`file-policy-write(context-ref: string, update: file-policy-update)` 当前只接受 `context-ref = "f"`，`update` 是结构化 record：

| 字段路径 | WIT 类型 | 含义 |
| --- | --- | --- |
| `update.rule-id` | string | 要写入的本地快路径规则 id。 |
| `update.decision` | control-verdict | `allow` 或 `deny`。 |
| `update.operation` | string | 必须等于当前命中操作。 |
| `update.path` | string | 必须等于当前命中文件路径。 |

写入成功返回 `file-policy-write-status.accepted`。
...
灰名单文件访问的超时 fallback 由 AcTrail 策略配置决定，默认拒绝。插件耗时较长时，应该把耗时放在明确需要同步决策的灰名单或显式策略命中路径上，避免拖慢普通快路径。
