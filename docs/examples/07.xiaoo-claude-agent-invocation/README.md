...
```bash
jq '[.resourceSpans[].scopeSpans[].spans[] | select(any(.attributes[]?; .key=="actrail.action.kind" and .value.stringValue=="command.invocation") and any(.attributes[]?; .key=="invocation.kind" and .value.stringValue=="agent"))] | length' \
...
第一条命令至少应输出 `1`。第二条命令必须非零。agent-labeled `command.invocation` span attributes 里应包含 `agent.child.command_line` 且其中有 `claude -p`；`process.parent.pid` 表示 Claude 的直接拉起进程，不要求一定是 xiaoO。
...
- AcTrail 会把发生 LLM 调用的 Claude 进程标记为 agent，并把对应 `command.invocation` 标注为 agent 调用。
