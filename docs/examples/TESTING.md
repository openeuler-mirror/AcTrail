...
Purpose: Java LangChain4j OpenAI-compatible HTTPS capture through launch-time JSSE Java-agent instrumentation.
...
Expected result: the traced `java -jar` workload reaches the real HTTPS provider, `actrailviewer payloads` shows complete outbound `TlsUserSpace/jsse` payload evidence, and OTEL export contains a complete `llm.request` span with the configured model and prompt.
...
Expected result: terminal output contains `ACTRAIL_AGENT_TREE_OK`, `agent_command_trace_id=<TRACE_ID>`, and `agent command e2e complete`. The script exports pretty OTLP JSON to `/tmp/actrail-xiaoo-claude-agent-invocation.otlp.json` and validates that it contains a seccomp-intent/eBPF-completion `process.exec` span for `claude -p`, an `llm.request` span for the same Claude process, a one-shot `agent.identity` whose evidence action points to that request, and a same-process `command.invocation`. The command parent is Claude's direct launcher and may be a shell or timeout wrapper.

This example intentionally enables TLS payload capture because agent identity is generated from LLM evidence, not from command names alone.
...
Expected result: terminal output contains `HIDDEN_AGENT_A_LLM_OK`, `HIDDEN_AGENT_XIAOO_OK`, `hidden_agent_trace_id=<TRACE_ID>`, `agent_a_pid=<PID>`, `xiaoo_pid=<PID>`, `script_b_parent_pid=<PID>`, and `hidden agent invocation e2e complete`. The script validates three OTEL facts before passing: the root `agent_a` process has a separate observed `agent.identity`, the hidden `xiaoo` process has its own observed `agent.identity`, and xiaoo's one-shot `command.invocation` uses the direct `script_b.sh -> xiaoo` parent/child relationship rather than an ancestor shortcut.
