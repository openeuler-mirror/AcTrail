...
如果目标 runtime 用 `writev` 或 `sendmsg` 发送 plain HTTP request，需要在同一配置段额外添加对应的 `payload_socket_seccomp_syscall`；这两个 syscall 的 payload 由 `iovec`/`msghdr` 描述，只走 seccomp user-read fallback，不走 BPF direct-copy。
