...
| `storage_sqlite_path` | AcTrail storage 路径；当 `storage_backend = sqlite` 时，采集到的 trace、membership、event、diagnostic 会写入这里 | 验证数据；`actraild stop` 不删除 |
| `storage_sqlite_busy_timeout_ms` | daemon 写 SQLite 时等待临时锁释放的最长时间；文件库使用 WAL 模式以支持 Web/viewer 并发读取 | 运行参数；来自配置文件 |
...
`suppressed_fd_max_entries` 控制 eBPF observation suppression fd map 容量。该 map 用于在采集热路径豁免 AcTrail 自身内部传输 fd，例如 `tls-sync` runtime event 通道，避免内部 IPC 被记录成 agent 行为。
...
| `payload_tls_source` | `tls-sync` 自动 plan 使用 `auto`；非 sync eBPF attach 使用 `shared-library` 或 `executable` 显式选择 attach 目标 |
| `payload_tls_resolver` | `tls-sync` 自动 plan 使用 `auto`；非 sync eBPF attach 使用 `openssl-symbols`、`boringssl-patterns`、`bun-static-boringssl`、`boringssl-static`、`rustls-symbol-map` 或 `go-pclntab` |
| `payload_tls_library` | `tls-sync` 自动 plan 使用 `auto`；非 sync eBPF attach 当前支持 `openssl`、`boringssl`、`rustls` 和 `go` |
| `payload_tls_library_path` | `tls-sync` 下 `auto` 表示不强制候选库路径；非 sync `shared-library` 模式下可显式指定 libssl 路径 |
...
| `payload_tls_java_agent_enabled` | 是否在 `tls-sync` launch 环境中注入 embedded JSSE Java agent；默认 `false`，Java LangChain4j HTTPS 示例显式设置为 `true` |
...
| `payload_stdio_stdin_storage_mode` | stdin payload 入库策略：`full`、`metadata-only` 或 `drop`；默认模板为 `full` |
| `payload_stdio_stdout_storage_mode` | stdout payload 入库策略；默认模板为 `drop`，用于避免 TUI 刷屏占用大量存储 |
| `payload_stdio_stderr_storage_mode` | stderr payload 入库策略；默认模板为 `metadata-only` |
...
| `payload_socket_seccomp_syscall` | socket large-payload user-read 监听 syscall；默认 full-monitor 配置包含 `write`、`writev`、`sendto` 和 `sendmsg`，覆盖线性和向量 socket 写入 |
...
| `application_http2_emit_data_preview` | L3 `data_content = preview` 时是否允许写入 UTF-8 HTTP/2 DATA preview；默认模板中 L3 DATA 内容仍为 `none` |
| `application_http2_max_data_preview_bytes` | L3 DATA preview 的最大字节数；同时作为 HTTP/2 body retention 的 LLM 分类探测窗口，窗口内仍不能证明是 LLM 的 stream 会进入 summary-only |
...
| `[semantic_retention.L0_llm_call]` | `request_content` | `none`、`shape`、`full_provider_json`；默认保留 provider request JSON/text |
...
| `[file_observation.tty]` | `operation` | 可重复配置；默认 `open`、`close`、`read`、`write` |
| `[file_observation.tty]` | `raw_event_retention` | `full`、`errors_only` 或 `summary`；默认 `errors_only`，成功 TTY raw event 不落 `events` 表，失败 raw event 保留 |
...
| `[file_observation.bulk_read]` | `min_unique_paths` | 触发 bulk read summary 的最小唯一路径数；默认 `128` |
...
`actrailviewer` 通过配置里的 `storage_sqlite_path` 读取 AcTrail storage，不需要先导出 JSON。把 `<N>` 替换为 `track-add` 返回的 trace id 数字：
...
`actrailweb` 是只读 UI，通过配置里的 `storage_sqlite_path` 读取 AcTrail storage，不需要先导出 JSON：
...
`tls-sync` 自动 plan 通常保持 `payload_tls_library_path = auto`，由 fast resolver 和动态绑定路径根据目标进程实际链接方式处理。非 sync `shared-library` attach 如果需要固定某个 libssl，可以显式配置绝对路径。

### `storage path does not exist`

说明配置里的 `storage_sqlite_path` 还没有生成，或者你用的 `--config` 不是启动 daemon 时的同一份配置。先确认 `actraild start` 使用的配置文件，再运行：
