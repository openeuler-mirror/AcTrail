...
| `payload_tls_capture_backend` | TLS 捕获后端；`seccomp-user-read` 由 daemon 在 syscall 暂停窗口读取 TLS buffer，`bpf-copy-seccomp-fallback` 先由 eBPF 直接复制不超过配置上限的 TLS write，超出时再走 seccomp 用户态读取；这两个后端都需要 `actrailctl launch` 在 child exec 前安装 seccomp notify |
| `payload_tls_source` | `shared-library` 表示 attach 动态 TLS shared library；`executable` 表示 attach 目标可执行文件 |
| `payload_tls_resolver` | `openssl-symbols` 按 TLS 符号 attach；`boringssl-patterns` 按外部 pattern 文件解析 executable offset；`bun-static-boringssl` 按带 build-id 的 Bun/BoringSSL symbol map 解析 executable offset；`boringssl-static` 按内置 x86_64/aarch64 BoringSSL read/write 关联入口检测 executable offset；`rustls-symbol-map` 按带 build-id 的 rustls symbol map 解析 executable offset |
| `payload_tls_library` | 当前支持 `openssl`、`boringssl` 和 `rustls` |
...
| `payload_tls_pattern_path` | `boringssl-patterns` resolver 的 pattern 文件路径；`bun-static-boringssl` 和 `rustls-symbol-map` resolver 的 symbol map 路径；`boringssl-static` 和未使用时写 `disabled` |
| `payload_tls_max_segment_bytes` | 单个 TLS BPF direct-copy segment 最大复制字节数；真实 Agent LLM 示例保持 `4095`，更大的请求 operation 通过 seccomp user-read 完整读取 |
| `payload_tls_max_operation_bytes` | daemon 在 seccomp 暂停窗口内对单次 TLS read/write 最多读取的明文字节数；默认 `4194304`，用于完整保留 4MB 以内的常见 LLM request |
...
| `payload_tls_seccomp_syscall` | TLS seccomp 后端监听的 syscall 集合；重复键配置多个 syscall，示例覆盖 `write`、`writev`、`sendto`、`sendmsg` |
| `seccomp_notify_reserved_listener_fd` | `actrailctl launch` child 在 exec 前放置 seccomp listener 的 reserved fd；parent 复制 fd 并注册给 daemon 后才放行 exec |
...
| `application_http2_max_frame_bytes` | 单个 HTTP/2 frame payload 的最大解析长度；超过会 fail-fast |
| `application_http2_max_connection_buffer_bytes` | 单个 trace/process/stream 的 HTTP/2 connection buffer 上限；超过会 fail-fast |
...
./target/release/actraild status --config docs/examples/01.quick-start/operator.conf
```
...
UI 会展示 trace 列表、Timeline、Process Tree、事件/进程/payload/诊断表、按域计数、搜索过滤和详情。Timeline 会把事件和 payload metadata 合并成按时间排序的 lanes；Process Tree 从 trace membership 派生父子进程关系。点击 payload 行会读取对应 payload segment 文本。
...
`actrailviewer events` 应出现 `Resource process_tree pid:<PID>`，Web UI 的 Resources tab 会显示 CPU%、RSS、VSZ、sample metadata，以及启用 `resource_metrics_include_system` 时的 host memory/load metadata。
