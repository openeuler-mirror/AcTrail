...
| `payload_tls_capture_backend` | TLS 捕获后端；当前 LLM TLS plaintext 推荐 `tls-sync`，由 `actrailctl launch` 在 exec 前准备 preload runtime、sync event socket 和 finder fast probe plan。旧的 `seccomp-user-read` / `bpf-copy-seccomp-fallback` TLS 后端用于特定调试或历史配置，不作为 examples 的默认 LLM TLS 路径 |
