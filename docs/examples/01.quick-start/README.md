...
| `[file_observation.bulk_read.fast_path]` | `enabled` | 是否对文件扫描命令启用内核侧 read 聚合；默认 `true`，匹配后 close/dup/fcntl 时输出 compact `read_summary` |
| `[file_observation.bulk_read.fast_path]` | `scanner_commands` | 触发 fast path 的进程 basename 列表；默认 `grep`、`egrep`、`fgrep`、`rg`、`find` |
| `[file_observation.bulk_read.fast_path]` | `process_max_entries` | fast path scanner 进程 map 容量；默认 `4096` |
| `[file_observation.bulk_read.fast_path]` | `fd_max_entries` | fast path fd 统计 map 容量；默认 `8192` |
| `[file_observation.bulk_read.fast_path]` | `pending_op_max_entries` | fast path read/close/dup/fcntl pending syscall map 容量；默认 `8192` |
