...
`scripts/install-release.sh` installs eight official packages under `${ACTRAIL_PLUGIN_DIR:-$HOME/.actrail/plugins}`: `otel-jsonl/` exposes selectable file and JSON-RPC HTTP(S) live OTLP JSON exporters, `otel-http/` provides live OTLP/HTTP export, `file-leakage/` provides post-trace leakage detection, `activity-anomaly/` reports LLM growth and running Agent commands as soon as they exceed the configured duration threshold and uses terminal analysis as a fallback, `tool-consecutive-failure-alert/` monitors repeated failing tool commands within a trace and reports consecutive-failure alerts, `file-policy-dynamic/` manages dynamic allow, deny, and gray file rules, `command-policy-dynamic/` manages dynamic executable routes with optional argv matching, and `network-policy-dynamic/` manages exact numeric INET connect endpoints or every port on one numeric IP. Installation only makes these packages discoverable. The generated startup list remains empty and disabled; use the local Plugins Web workspace or the plugin CLI to load a package explicitly.
...
The dynamic command-policy plugin uses the parallel `command-policy.rules.apply:kind=<allow|deny|gray>,path=<absolute-path-or-scope>` grants. A grant may end in `/**`, but each published rule still names one exact tracee-namespace executable path. Command governance additionally requires `[command_control].enabled = true`, `[seccomp_notify].enabled = true`, and a launch capture profile containing both `proc-lifecycle` and `enforcement-command-execution-seccomp`. This capability is launch-only; `track-add` rejects it because an existing process cannot be retrofitted with the required seccomp listener.
