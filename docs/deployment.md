...
`scripts/install-release.sh` installs six official packages under `${ACTRAIL_PLUGIN_DIR:-$HOME/.actrail/plugins}`: `otel-jsonl/` exposes selectable file and JSON-RPC HTTP(S) live OTLP JSON exporters, `file-leakage/` provides post-trace leakage detection, `activity-anomaly/` reports LLM growth and running Agent commands as soon as they exceed the configured duration threshold and uses terminal analysis as a fallback, `tool-consecutive-failure-alert/` monitors repeated failing tool commands within a trace and reports consecutive-failure alerts, `file-policy-dynamic/` manages dynamic allow, deny, and gray file rules, and `command-policy-dynamic/` manages dynamic executable routes with optional argv matching. Installation only makes these packages discoverable. The generated startup list remains empty and disabled; use the local Plugins Web workspace or the plugin CLI to load a package explicitly.
