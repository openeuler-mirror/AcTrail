...
> 本文定义插件作者将非标准 LLM request body 或 SSE event data 解码为 AcTrail 标准输入所需的 ABI。
...
WASM core module 插件还需要遵守 [WASM Core Module ABI](wasm-core-module.md) 中的 `memory`、`actrail_alloc` 和可选 `actrail_plugin_init` 约定。`llm-codec` 当前只支持 WASM core module，不支持 WIT component。
...
`host.capabilities` 通常为空。codec 输入来自 AcTrail 已经捕获并正在投影的 HTTP/SSE 数据，不需要通过 `payload-read` hostcall 再读 payload。插件加载和授权见 [管理插件](../../operations/plugins/manage.md)。
...
- 不需要解码的输入应返回 `no_match`，而非空 decoded body。
