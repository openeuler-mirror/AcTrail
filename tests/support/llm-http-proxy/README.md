...
`provider_proxy.py` is a local OpenAI-compatible reverse provider shim for manual and automated agent tests. By default it listens on `http://127.0.0.1:18098`, reads the upstream credential from `DEEPSEEK_API_KEY`, and forwards requests to `https://api.deepseek.com` with `Authorization: Bearer`.
