...
```text
tests/v2/common/test_suites/local_maas_server/
├── README.md
├── server.py
├── config.py
│
...
server.py
    ↓
parse_cli_args（subcommand 选择入口）
    ↓
    ├── ScenarioReplayServer（replay）
    ├── TransportServer（transport）
    └── ScenarioRecordServer（record，继承 TransportServer）
            ↓
