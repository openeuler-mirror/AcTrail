...
运行中的 server 通过 `GET /` 或 `GET /help`（`POST` 同样可用）返回模式相关的
使用说明与示例（replay/transport/record 各自不同），内容与启动日志共用同一套
help message：

```bash
curl http://127.0.0.1:42117/help
```

## 运行模式

subcommand 选择 server 入口：

- `replay`：`ScenarioReplayServer`，按剧本播放；
- `transport`：`TransportServer`，LLM 透明代理，必须提供
  上游配置。解析顺序：`--transport-config <file.json>`（`base_url` /
  `api_key` / 可选 `model`）→ 环境变量 `LOCAL_MAAS_UPSTREAM_URL` /
  `LOCAL_MAAS_UPSTREAM_API_KEY` / `LOCAL_MAAS_PROTOCOL`（可选
  `LOCAL_MAAS_UPSTREAM_MODEL`）→ `DEEPSEEK_API_KEY`（启动时探测
  `https://api.deepseek.com/models` 并配置上游 model）。全部缺失时启动
  失败；`LOCAL_MAAS_PROTOCOL` 目前只支持 `openai`；
- `record`：`ScenarioRecordServer`（继承 `TransportServer`），录制模式，
  见 [录制模式](docs/record-mode.md)。

三类 server 共享 `LocalMaaSServer` 基类的端口监听、生命周期与启动日志逻辑。
...
- `bash-home-loop.json`
- `action_pools/random-light-exec.json`
- `action_pools/adaptive-tool-or-message-loop.json`
- `action_pools/random-file-operation.json`
- `action_pools/reasoning-length-loop.json`
- `action_pools/short-reasoning-sequential-cycle.json`
- `action_pools/long-reasoning-short-operation.json`
...
```bash
python3 tests/v2/common/test_suites/local_maas_server/server.py --help
```
