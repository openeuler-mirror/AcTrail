...
class HTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path in self._HELP_PATHS:
            self._send_response(self._help_response())
            return
        if self.application.supports_management("GET", path):
            self._send_response(
                self.application.handle_management("GET", path, None)
            )
            return
    ...
    def do_POST(self) -> None:
        ...
        if path in self._HELP_PATHS:
            self._send_response(self._help_response())
            return
        if self.application.supports_reset_path(path):
            self.application.reset()
            self._send_response(
                self._json_response(
                    HTTPStatus.OK,
                    {"status": "ok", "reset": True},
                )
            )
            return
        if self.application.supports_management("POST", path):
            try:
                document = self._read_json_request()
                result = self.application.handle_management(
                    "POST", path, document
                )
                self._send_response(
                    result,
                    close_connection=result.status >= 400,
                )
        ...
        if not self.application.supports_path(path):
            ...
            return
        try:
            self.application.verify_request(self.headers)
            document = self._read_json_request()
            result = self.application.handle_post(
                path, document, self.headers
            )
    ...
    def _help_response(self) -> ProtocolResponse:
        return ProtocolResponse(
            status=HTTPStatus.OK,
            media_type="text/plain; charset=utf-8",
            body=self.application.help_text(
                self.connection_server.origin
            ).encode("utf-8"),
        )
