...
from .task import ProbeClaudeMcpTask


class ProbeClaudeMcpCase(TestCase):
    def __init__(self, config: ProbeClaudeMcpConfig) -> None:
        self._config = config

    def run(self, test_context: TestingContextSingleton) -> TestResult:
        results: dict[str, TestResult] = {}
        runtime: ActrailRuntime | None = None
        task: ProbeClaudeMcpTask | None = None
        try:
            ...
            launch = task.run(test_context.output)
            ...
            return TestResult(
                ...
                "Claude stdio MCP tool-call semantic recognition",
                results,
            )
