...
python3 -u "$ACTRAIL_XIAOO_PROVIDER_SCRIPT" \
  ...
  --local-tool-command "printf '%s\\n' ACTRAIL_EXECUTION_AGENT_WRITE_OK > $WRITE_OUTPUT" \
...
grep -Fq 'ACTRAIL_EXECUTION_AGENT_READ_INPUT' "$READ_OUTPUT" \
  || fail "real xiaoO did not perform the required file read tool call"
grep -Fq 'ACTRAIL_EXECUTION_AGENT_WRITE_OK' "$WRITE_OUTPUT" \
...
echo "ACTRAIL_EXECUTION_NAMED_ROOT_OK child_exit=$xiaoo_rc"
echo "ACTRAIL_EXECUTION_AGENT_TOOLS_OK instance=$ACTRAIL_XIAOO_INSTANCE"
