...
class ProbeXiaooLLMRunner:
    def run(self) -> int:
        if os.geteuid() != 0:
            print("failed: this eBPF E2E must run as root")
            return 1
        arguments = self._arguments()
        config = ProbeXiaooLLMConfig.from_environment(REPO, arguments.bin_dir)
        result = ProbeXiaooLLMCase(config).run(TestingContextSingleton())
        self._print_result("probe_xiaoo_llm", result, 0)
...
if __name__ == "__main__":
    raise SystemExit(ProbeXiaooLLMRunner().run())
