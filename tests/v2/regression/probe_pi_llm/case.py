...
class ProbePiLLMCase(TestCase):
    def __init__(self, config: ProbePiLLMConfig):
        self._config = config
    ...
    def run(self, test_context: TestingContextSingleton) -> TestResult:
        ...
        try:
            ...
            try:
                task = ProbePiLLMTask(self._config, runtime)
            ...
            if not test_context.check_agent_availability(
                ...
                return TestResult(
                    TestStatus.SKIPPED,
                    "pi external availability check failed",
                )
            ...
            results["launch"] = TestResult(
                TestStatus.PASSED,
                "actrailctl launch and pi exited successfully",
            )
            ...
            results["answer_marker"] = TestResult(
                TestStatus.PASSED,
                f"pi stdout answer contains {task.marker}",
            )
