...
class ToolFrequentFailureAlertEnvironment:
    def __init__(
        self,
        config: ToolFrequentFailureAlertConfig,
        output: TestOutput,
    ):
        self.config = config
        self.output = output
    ...
    def installed_package_dir(self) -> Path:
        return self.plugin_root / "tool-frequent-failure-alert"

    @property
    def installed_manifest(self) -> Path:
        return self.installed_package_dir / "tool-frequent-failure-alert.plugin.toml"

    def prepare(self) -> None:
        self.runtime.prepare()
        self.load_repo_plugin()

    def load_repo_plugin(self, plugin_config: Path | None = None) -> None:
        if self._plugin_loaded:
            return
        load = self.runtime.run_checked(
            self.plugin_load_command(plugin_config=plugin_config)
        )
    ...
    def plugin_load_command(
        ...
        *,
        ...
        return self._plugin_load_command(
            ...
            plugin_config or self.config.plugin_config,
        )
