...
| Agent invocation E2E | `python3 tests/process/agent-invocation/run_e2e.py` | 真实 xiaoO 调用前台 Claude；验证 agent identity 由 Claude 子进程 LLM evidence 推导，而非命令名硬编码。 | launch 输出 `ACTRAIL_AGENT_TREE_OK`；OTEL 中有 Claude `process.exec`、同进程 `llm.request` 与 `agent.identity`、Claude 直接子 Bash `command.invocation`，以及同进程的一次性 Claude `command.invocation`。 |
...
| Hidden agent invocation | `python3 tests/process/hidden-agent-invocation/run_e2e.py` | 编译 `agent_a`，形成 `agent_a -> script_b.sh -> xiaoo --cli run`，验证隐藏 agent identity 和直接父子关系。 | `agent_a` 与 xiaoO 均有真实 LLM evidence 和独立 identity；xiaoO command 只记录 `script_b.sh` 直接父进程，不生成 `agent_a -> xiaoo` 祖先快捷边。 |
