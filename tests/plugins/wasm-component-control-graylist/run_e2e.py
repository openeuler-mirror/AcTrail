#!/usr/bin/env python3
"""Run the WIT component ControlDecider fanotify graylist E2E."""
...
FIXTURE_DIR = ROOT / "tests/plugins/wasm-component-control-graylist"
GRAYLIST_DIR = ROOT / "tests/plugins/control-graylist"
MANIFEST = FIXTURE_DIR / "component-allow.plugin.toml"
UNSUPPORTED_GRANT_MANIFEST = FIXTURE_DIR / "component-env-read-unsupported.plugin.toml"
INSTANCE = "wasm.component-graylist-allow"
...
def wait_for_enforcement_events(
    ...
    raise RuntimeError("actrailviewer did not show expected component graylist plugin events")
...
def expect_load_failure(command: list[str], expected: str) -> None:
    result = subprocess.run(command, text=True, capture_output=True, check=False)
    combined = result.stdout + result.stderr
    if result.returncode == 0:
        raise RuntimeError(f"expected load failure but command succeeded: {' '.join(command)}")
    if expected not in combined:
        raise RuntimeError(
            f"load failure missed expected text {expected!r}\nstdout={result.stdout}\nstderr={result.stderr}"
        )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="actrail-component-control-graylist-e2e-") as raw_tmp:
        tmp = Path(raw_tmp)
        ...
        plugin_config = tmp / "component-control.config.toml"
        GRAYLIST.write_text(allowed, "allowed\n")
        GRAYLIST.write_text(gray, "gray\n")
        GRAYLIST.write_text(denied, "denied\n")
        GRAYLIST.write_text(plugin_config, 'mode = "component-control-ok"\n')
        GRAYLIST.write_text(
            rules,
            "\n".join(
                [
        ...
        try:
            ...
            expect_load_failure(
                [
                    ...
                    "--grant",
                    "env-read:ACTRAIL_COMPONENT_CONTROL_SECRET",
                    "--instance",
            ...
            try:
                ...
                GRAYLIST.run_checked(
                    [
                        ...
                        str(pid),
                        "--name",
                        "component-control-graylist-plugin-e2e",
                    ]
                )
                ...
                if "gray=ok" not in agent_output:
                    raise RuntimeError("agent did not confirm gray component plugin allow")
                ...
                if status.get("purpose") != "control-decider":
                    raise RuntimeError(f"component control plugin status missed purpose\n{status}")
                if status.get("runtime") != "wasm":
                    raise RuntimeError(f"component control plugin status missed runtime\n{status}")
                if status.get("host_grants") != "none":
                    raise RuntimeError(f"component control plugin should not have host grants\n{status}")
                observed = int(status.get("observed_records", "0"))
                if observed <= 0:
                    raise RuntimeError(f"component control plugin did not record decisions\n{status}")
                if status.get("last_error", "") not in ("", "none"):
                    raise RuntimeError(f"component control plugin reported last_error\n{status}")
                GRAYLIST.run_checked(
                    [
                        str(actraild),
                        "--config",
    ...
    print(f"wasm_component_control_graylist_plugin_instance={INSTANCE}")
    print("wasm_component_control_graylist_plugin_observed_records=1")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"component control graylist e2e failed: {error}", file=sys.stderr)
        raise SystemExit(1)
