...
RUN_DIR = Path("/tmp/actrail-plugin-wasm-env-read")
SOURCE_CONFIG = ROOT / "tests/plugins/otel-jsonl/operator.conf"
CONFIG = RUN_DIR / "operator.conf"
SOCKET_PATH = RUN_DIR / "actraild.sock"
MANIFEST = ROOT / "tests/plugins/wasm-env-read/env-read.plugin.toml"
NO_CAPABILITY_MANIFEST = ROOT / "tests/plugins/wasm-observation/count.plugin.toml"
INSTANCE = "wasm.env-read"
ENV_NAME = "ACTRAIL_PLUGIN_ENV_SECRET"
ENV_VALUE = "allowed-secret"
MARKER = "ACTRAIL_WASM_ENV_READ_PLUGIN_E2E"
...
def assert_ungranted_load_fails(actraild: Path) -> None:
    ...
    if load.returncode == 0:
        raise RuntimeError("env-read plugin load unexpectedly succeeded without grant")
...
def assert_unrequested_grant_fails(actraild: Path) -> None:
    ...
    if load.returncode == 0:
        raise RuntimeError("plugin load unexpectedly accepted an unrequested env-read grant")
    missing = [value for value in ("plugin_capability", "did not request", "env-read") if value not in load.stdout]
    if missing:
        raise RuntimeError(f"unrequested grant error missed {missing}\n{load.stdout}")
...
def wait_for_observed_record(actraild: Path) -> int:
    ...
    observed = int(status.get("observed_records", "0"))
    if observed <= 0:
        raise RuntimeError(f"env-read plugin did not observe batches\n{status}")
    if status.get("last_error", "") not in ("", "none"):
        raise RuntimeError(f"env-read plugin reported last_error\n{status}")
    return observed
...
def assert_grant_is_auditable(actraild: Path) -> None:
    ...
    if status.get("host_grants") != expected:
        raise RuntimeError(f"plugin status did not expose host grant {expected}\n{status_raw}")
    list_raw = run([str(actraild), "--config", str(CONFIG), "plugin", "list"]).stdout
    if expected not in list_raw:
        raise RuntimeError(f"plugin list did not expose host grant {expected}\n{list_raw}")
...
def main() -> int:
    ...
    try:
        ...
        print(f"wasm_env_read_plugin_instance={INSTANCE}")
        print(f"wasm_env_read_plugin_observed_records={observed}")
        return 0
    finally:
