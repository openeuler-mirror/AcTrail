#!/usr/bin/env python3
"""Run the graylist shared plugin instance admission E2E."""
...
INSTANCE = "wasm.graylist-shared-instance"
...
def wait_for_shared_instance_audit(
    actrailctl: Path,
    actrailviewer: Path,
    config: Path,
    attempts: int,
    ...
    raise RuntimeError("actrailviewer did not show expected shared-instance fallback audit")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="actrail-control-shared-instance-e2e-") as raw_tmp:
        tmp = Path(raw_tmp)
        ...
        helpers.write_text(
            ...
            "\n".join(
                [
                    f"first-gray gray sync-plugin {INSTANCE} timeout-ms 3000 concurrency 1 fallback deny open {first_gray}",
                    f"second-gray gray sync-plugin {INSTANCE} timeout-ms 3000 concurrency 1 fallback deny open {second_gray}",
                    "",
                ]
            ),
        )
        ...
        try:
            ...
            track_agent(actrailctl, config, first_pid, "control-shared-instance-first")
            release_agent(first)
            time.sleep(0.25)

            second = start_agent(agent_script, second_gray)
            second_pid = helpers.read_agent_pid(second, args.agent_timeout_sec)
            track_agent(actrailctl, config, second_pid, "control-shared-instance-second")
            ...
            if "gray=permission_denied" not in second_output:
                raise RuntimeError(
                    f"second agent was not denied by shared-instance fallback\n{second_output}"
                )
            ...
            viewer_output = wait_for_shared_instance_audit(
                actrailctl,
                actrailviewer,
                config,
                args.drain_attempts,
    ...
    print(f"control_shared_instance_plugin_instance={INSTANCE}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"control shared-instance e2e failed: {error}", file=sys.stderr)
        raise SystemExit(1)
