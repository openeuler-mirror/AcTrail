#!/usr/bin/env python3
"""Run command-execution ControlDecider E2E with a real launched workload."""
...
def require_root() -> None:
    if os.geteuid() != 0:
        raise RuntimeError("command-execution control E2E requires root for seccomp setup")
...
def operator_config(tmp: Path, rules_path: Path) -> str:
    return f"""# Generated by tests/plugins/command-execution/run_e2e.py
[control]
        ...
        try:
            ...
            run_checked(
                [
                    ...
                    "load",
                    "--manifest",
                    str(FIXTURE_DIR / "deny-command.plugin.toml"),
                    "--instance",
                    INSTANCE,
                ]
            )
            agent = subprocess.Popen(
                [
                    ...
                    "launch",
                    "--name",
                    "command-execution-control-plugin-e2e",
                    "--",
                    sys.executable,
                    str(agent_script),
                    "--allowed-command",
