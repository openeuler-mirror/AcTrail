#!/usr/bin/env python3
"""Run the graylist sync-plugin concurrency admission E2E."""
...
MANIFEST = TIMEOUT_FIXTURE_DIR / "timeout.plugin.toml"
PLUGIN_CONFIG = TIMEOUT_FIXTURE_DIR / "timeout.config.toml"
INSTANCE = "wasm.graylist-concurrency"
...
def wait_for_concurrency_audit(
    actrailctl: Path,
    actrailviewer: Path,
    config: Path,
    attempts: int,
    ...
    expected = [
        ...
        "fallback_reason=concurrency_limit",
        "plugin_inflight=1",
    ]
    ...
    raise RuntimeError("actrailviewer did not show expected concurrency fallback audit")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="actrail-control-concurrency-e2e-") as raw_tmp:
        tmp = Path(raw_tmp)
        ...
        helpers.write_text(
            rules,
            f"gray-file gray sync-plugin {INSTANCE} timeout-ms 3000 concurrency 1 fallback deny open {gray}\n",
        )
        ...
        try:
            ...
            first = start_agent(agent_script, gray)
            first_pid = helpers.read_agent_pid(first, args.agent_timeout_sec)
            track_agent(actrailctl, config, first_pid, "control-concurrency-first")
            release_agent(first)
            time.sleep(0.25)

            second = start_agent(agent_script, gray)
            second_pid = helpers.read_agent_pid(second, args.agent_timeout_sec)
            track_agent(actrailctl, config, second_pid, "control-concurrency-second")
            ...
            if "gray=permission_denied" not in second_output:
                raise RuntimeError(f"second agent was not denied by concurrency fallback\n{second_output}")
            ...
            viewer_output = wait_for_concurrency_audit(
                actrailctl,
                actrailviewer,
                config,
                args.drain_attempts,
            ...
            if "plugin_error=wasm_runtime: wasm timeout" not in viewer_output:
    ...
    print(f"control_concurrency_plugin_instance={INSTANCE}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"control concurrency e2e failed: {error}", file=sys.stderr)
        raise SystemExit(1)
