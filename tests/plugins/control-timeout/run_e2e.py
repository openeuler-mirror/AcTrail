#!/usr/bin/env python3
"""Run the WASM ControlDecider wall-clock timeout E2E."""
...
FIXTURE_DIR = ROOT / "tests/plugins/control-timeout"
MANIFEST = FIXTURE_DIR / "timeout.plugin.toml"
PLUGIN_CONFIG = FIXTURE_DIR / "timeout.config.toml"
INSTANCE = "wasm.graylist-timeout"
...
def wait_for_agent_result(process: subprocess.Popen[str], timeout_sec: float) -> str:
    ...
    try:
        ...
        raise RuntimeError("agent did not receive graylist timeout fallback response") from error
    output = "".join(lines)
    if exit_code != 0:
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="actrail-control-timeout-e2e-") as raw_tmp:
        tmp = Path(raw_tmp)
        ...
        helpers.write_text(
            rules,
            f"gray-file gray sync-plugin {INSTANCE} timeout-ms 50 concurrency 1 fallback deny open {gray}\n",
        )
        ...
        try:
            ...
            try:
                ...
                if "gray=permission_denied" not in output:
                    raise RuntimeError(f"gray timeout fallback did not deny access\n{output}")
                ...
                if dropped <= 0:
                    raise RuntimeError(f"timeout was not recorded as dropped\n{status}")
                last_error = status.get("last_error", "")
                if "timeout" not in last_error:
                    raise RuntimeError(f"timeout error was not reported\n{status}")
            finally:
                helpers.stop_process(agent)
        finally:
            helpers.stop_process(daemon)

    print(f"control_timeout_plugin_instance={INSTANCE}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"control timeout e2e failed: {error}", file=sys.stderr)
        raise SystemExit(1)
