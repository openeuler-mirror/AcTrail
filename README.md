...
The install script checks build dependencies, installs the actrailweb frontend dependencies with `npm ci`, builds missing release binaries, and copies them into the destination directory. It also installs the official plugin packages, including the built-in `otel-jsonl` exporter descriptor, under `${ACTRAIL_PLUGIN_DIR:-$HOME/.actrail/plugins}`. Installation makes plugins discoverable but does not load them; use the Plugins Web workspace to refresh and explicitly load one. The script uses `sudo` only for copies whose destination requires elevated permissions. Use another binary or plugin directory when needed, and keep the configured plugin discovery path aligned with the user that runs `actrailweb`.
