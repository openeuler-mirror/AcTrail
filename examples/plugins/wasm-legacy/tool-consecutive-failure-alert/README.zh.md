...
- `actrail_tool_consecutive_failure_alert.wasm`：已编译的 component artifact。
- `alert-schema.json`：告警 payload JSON Schema。
- `src/lib.rs`：Rust 源码。
...
插件使用内置默认配置，无需额外配置文件。如需自定义，可创建 `config.toml` 并在加载时通过 `--plugin-config` 传入：
...
#加载插件
./target/release/actraild plugin load   --manifest examples/plugins/wasm-legacy/tool-consecutive-failure-alert/plugin.toml   --instance tool-alert.test   --grant alert-write
#运行agent 
