...
const EXPECTED_CONFIG: &[u8] = b"mode = \"component-control-ok\"\n";
...
impl ControlGuest for Component {
    fn decide(request: DecisionRequest) -> Result<DecisionResponse, String> {
        ...
        if request.decision_id.is_empty()
            ...
            || request.actor_process_identity.pid == 0
            || request.target_summary.is_empty()
        {
            return Err("missing minimal decision fields".to_string());
        }
        ensure_config()?;
        Ok(DecisionResponse {
            verdict: ControlVerdict::Allow,
            scope: DecisionScope::Once,
            reason_code: Some("component-gray-allow".to_string()),
            reason_message: Some("component control allowed gray file".to_string()),
        })
    }
}
