...
mod tests {
        ...
        for (group, group_exchanges) in &groups {
            ...
            while i < sorted.len() {
                ...
                while run_end + 1 < sorted.len()
                    && similar_requests(sorted[run_end], sorted[run_end + 1], tolerance_per_mille)
                {
                    run_end += 1;
                }
                let run_length = run_end - run_start + 1;
                if run_length >= min_repeat_count {
                    let rep = sorted[run_start];
                    let last_in_run = sorted[run_end];
                    findings.push((
                        group.process_id.clone(),
                        group.model.clone(),
                        run_length,
    ...
    #[test]
    fn repeated_similar_detects_multiple_runs_in_aabbc_pattern() {
        let mut exchanges = Vec::new();
        let bytes_sequence = [1000, 1000, 2000, 2000, 3000];
        for (i, &bytes) in bytes_sequence.iter().enumerate() {
            exchanges.push(MockExchange {
                call_action_id: format!("call-{i}"),
                request_action_id: format!("req-{i}"),
                response_action_id: Some(format!("res-{i}")),
        ...
        let findings = detect_repeated_similar(&exchanges, 2, 2, 50);
        assert_eq!(findings.len(), 2);
        assert_eq!(findings[0].2, 2);
        assert_eq!(findings[0].3, "req-0");
        assert_eq!(findings[0].4, 1000);
        assert_eq!(findings[1].2, 2);
        assert_eq!(findings[1].3, "req-2");
        assert_eq!(findings[1].4, 2000);
    }
    ...
    impl IncrementalCursor {
        fn ingest_snapshot(&mut self, request_action_ids: &[&str]) -> Result<Vec<String>, String> {
            let start = self.next_offset.saturating_sub(1);
            let mut rows = request_action_ids.iter().skip(start);
            if self.next_offset > 0 {
                let checkpoint = rows
                    .next()
                    .ok_or_else(|| "incremental checkpoint disappeared".to_string())?;
                if self.last_request_action_id.as_deref() != Some(*checkpoint) {
                    return Err("incremental checkpoint changed".to_string());
                }
            }
            let mut added = Vec::new();
            for action_id in rows {
                added.push((*action_id).to_string());
                self.next_offset += 1;
                self.last_request_action_id = Some((*action_id).to_string());
            }
            Ok(added)
        }
    }
    ...
    #[test]
    fn incremental_cursor_does_not_reinsert_duplicate_snapshot() {
        let mut cursor = IncrementalCursor::default();
        assert_eq!(
            cursor.ingest_snapshot(&["req-1", "req-2"]).unwrap(),
            vec!["req-1", "req-2"]
        );
        assert!(
            cursor
                .ingest_snapshot(&["req-1", "req-2"])
                .unwrap()
                .is_empty()
        );
        assert_eq!(
            cursor
                .ingest_snapshot(&["req-1", "req-2", "req-3"])
                .unwrap(),
            vec!["req-3"]
        );
    }
    ...
    #[test]
    fn incremental_cursor_rejects_reordered_history() {
        let mut cursor = IncrementalCursor::default();
        cursor.ingest_snapshot(&["req-1", "req-2"]).unwrap();
        assert!(
            cursor
                .ingest_snapshot(&["req-0", "req-1", "req-2"])
                .is_err()
        );
    }

    #[test]
    fn detector_windows_evict_old_values() {
        let mut frequency = VecDeque::new();
        for timestamp in [0_u64, 10, 20] {
            frequency.push_back(timestamp);
            let window_start = timestamp.saturating_sub(15);
            while frequency
                .front()
                .is_some_and(|value| *value < window_start)
            {
                frequency.pop_front();
            }
        }
        assert_eq!(frequency, VecDeque::from([10, 20]));

        let mut response_complete = VecDeque::new();
        for value in [false, true, false, true, true] {
            response_complete.push_back(value);
            while response_complete.len() > 3 {
                response_complete.pop_front();
            }
        }
        assert_eq!(response_complete, VecDeque::from([false, true, true]));

        let mut request_body_bytes = VecDeque::new();
        for value in [100_u64, 200, 300, 400] {
            request_body_bytes.push_back(value);
            while request_body_bytes.len() > 2 {
                request_body_bytes.pop_front();
            }
        }
        assert_eq!(request_body_bytes, VecDeque::from([300, 400]));
    }

    #[derive(Default)]
    struct SimilarEpisodeWindow {
        requests: VecDeque<u64>,
        episode_alerted: bool,
    }

    impl SimilarEpisodeWindow {
        fn observe(
            &mut self,
            request_body_bytes: u64,
            similarity_window: usize,
            ...
            let continues = self.requests.back().is_some_and(|previous| {
                let (larger, smaller) = if *previous >= request_body_bytes {
                    (*previous, request_body_bytes)
                } else {
                    (request_body_bytes, *previous)
                };
                larger == smaller
                    || (smaller > 0
                        && (larger - smaller) * 1000 <= larger * tolerance_per_mille)
            });
            if !continues {
                self.requests.clear();
                self.episode_alerted = false;
            }
            self.requests.push_back(request_body_bytes);
            while self.requests.len() > similarity_window {
                self.requests.pop_front();
            }
            if self.requests.len() >= min_repeat_count && !self.episode_alerted {
    ...
    #[test]
    fn repeated_similar_queue_alerts_again_after_normal_request() {
        let mut state = SimilarEpisodeWindow::default();
        let alerts = [1000, 1000, 3000, 2000, 2000]
            .into_iter()
            .filter(|bytes| state.observe(*bytes, 2, 2, 100))
            .count();
        assert_eq!(alerts, 2);
    }

    #[test]
    fn repeated_similar_queue_does_not_repeat_within_one_episode() {
        let mut state = SimilarEpisodeWindow::default();
        let alerts = [1000, 1000, 1000, 1000, 1000]
            .into_iter()
            .filter(|bytes| state.observe(*bytes, 2, 2, 100))
            .count();
        assert_eq!(alerts, 1);
        assert_eq!(state.requests, VecDeque::from([1000, 1000]));
    }

    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    enum TestResponseStatus {
        Pending,
        Success,
        Error,
        Unknown,
    ...
    fn drain_terminal_responses(
        pending: &mut VecDeque<PendingResponse>,
    ) -> Vec<TestResponseStatus> {
        let mut outcomes = Vec::new();
        while pending
            .front()
            .is_some_and(|response| response.status != TestResponseStatus::Pending)
        {
    ...
    #[test]
    fn pending_response_is_not_counted_until_terminal() {
        let mut pending = VecDeque::from([PendingResponse {
            status: TestResponseStatus::Pending,
        }]);
        assert!(drain_terminal_responses(&mut pending).is_empty());

        pending[0].status = TestResponseStatus::Success;
        assert_eq!(
            drain_terminal_responses(&mut pending),
            vec![TestResponseStatus::Success]
        );
        assert!(pending.is_empty());
    }

    #[test]
    fn resolved_responses_are_released_in_request_order_once() {
        ...
        assert_eq!(
            drain_terminal_responses(&mut pending),
            vec![TestResponseStatus::Success, TestResponseStatus::Error]
        );
        assert!(drain_terminal_responses(&mut pending).is_empty());
    }

    #[test]
    fn unknown_response_is_terminal_but_not_an_error() {
        let status = TestResponseStatus::Unknown;
        assert_ne!(status, TestResponseStatus::Pending);
        assert_ne!(status, TestResponseStatus::Error);
    }
}
