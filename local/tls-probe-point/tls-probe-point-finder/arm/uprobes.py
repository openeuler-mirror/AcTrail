"""ARM64 runtime uprobe verification for TLS probe point candidates."""
...
def trace_addresses(args) -> None:
    ...
    try:
        for index, address in enumerate(addresses):
            ...
            fetch_args = "x0=%x0 x1=%x1 x2=%x2"
            if args.fetch_x1_string:
                fetch_args = f"{fetch_args} x1s=+0(%x1):ustring"
            event_line = f"p:{args.group}/p{index} {binary}:0x{file_offset:x} {fetch_args}\n"
...
def print_hit_summary(trace: str) -> None:
    pattern = re.compile(r":\s+(p\d+):.*?\bx2=(0x[0-9a-fA-F]+|0x)")
    ...
    for event, values in sorted(hits.items(), key=lambda item: (-len(item[1]), item[0])):
        ...
        max_x2 = "none" if not numeric else f"0x{max(numeric):x}"
        print(f"{event} count={len(values)} max_x2={max_x2}")
