...
pub(super) struct ProcessDisplayIndex {
    processes: BTreeMap<ProcessIdentity, ProcessRecord>,
}

impl ProcessDisplayIndex {
    pub(super) fn load(storage: &dyn StorageBackend) -> Result<Self, String> {
        let processes = storage
            .list_process_records()
            .map_err(|error| {
                format!(
                    "list process records failed: {}: {}",
    ...
    pub(super) fn render_trace(&self, trace: &TraceRecord) -> String {
        let root_process = self.processes.get(&trace.root_process_identity);
        self.render_with_root_process(trace, root_process)
    }

    pub(super) fn host_pid(&self, identity: ProcessIdentity) -> Option<u32> {
        self.processes
            .get(&identity)?
            .host
            .as_ref()
    ...
    fn render_with_root_process(
        &self,
        trace: &TraceRecord,
        root_process: Option<&ProcessRecord>,
    ) -> String {
        ...
        json::field(
            &mut output,
            "root_process_id",
            &json::number(trace.root_process_identity.get()),
        );
        output.push(',');
        json::field(
            &mut output,
            ...
            &json::optional_number(
                root_process.and_then(|process| process.host.as_ref().map(|host| host.pid)),
            ),
        );
        output.push(',');
        json::field(
