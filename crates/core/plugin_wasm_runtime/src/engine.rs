...
use plugin_system::{
    PluginHostGrants, PluginHostcallMetrics, PluginHostcallMetricsSource, PluginManifest,
    PluginPayloadReadMetrics, PluginRuntimeError,
};
...
pub(crate) use state::{
    ControlContextSnapshot, PostTraceCallLimits, WasmHostLimits, WasmStoreState,
};
...
pub(crate) const DEFAULT_HOSTCALL_ALERT_PAYLOAD_MAX_BYTES: u32 = 64 * 1024;
...
pub(crate) fn host_limits(manifest: &PluginManifest) -> Result<WasmHostLimits, PluginRuntimeError> {
    ...
    let alert_payload_max_bytes = manifest
        .hostcall_limits
        .alert
        .payload_max_bytes
        .unwrap_or(DEFAULT_HOSTCALL_ALERT_PAYLOAD_MAX_BYTES);
    Ok(WasmHostLimits {
        ...
        alert_payload_max_bytes: usize::try_from(alert_payload_max_bytes).map_err(|error| {
            PluginRuntimeError::new(
                "wasm_runtime",
                format!("alert payload hostcall byte limit overflow: {error}"),
            )
        })?,
    })
}
...
pub(crate) fn limited_store(
    ...
    let mut store = Store::new(
        engine,
        WasmStoreState::new(memory_max_bytes, host_grants, host_limits, hostcall_metrics),
    );
