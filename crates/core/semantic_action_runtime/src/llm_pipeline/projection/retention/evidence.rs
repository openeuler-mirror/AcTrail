...
pub(in crate::llm_pipeline) fn payload_aggregate_evidence(
...
pub(in crate::llm_pipeline) fn payload_snapshot_evidence(
...
pub(in crate::llm_pipeline) fn insert_payload_span_attributes(
...
pub(in crate::llm_pipeline) fn insert_payload_snapshot_attributes(
    attributes: &mut BTreeMap<String, String>,
    snapshot: &EvidenceSnapshot,
) {
    let Some(first) = snapshot.first.as_ref() else {
        return;
    };
    let last = snapshot.last.as_ref().unwrap_or(first);
    attributes.insert(
        attrs::payload_aggregate::FIRST_SEGMENT_ID.to_string(),
        first.segment_id.get().to_string(),
    );
