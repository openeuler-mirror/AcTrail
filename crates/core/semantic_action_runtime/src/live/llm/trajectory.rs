...
use crate::payload_projection::llm::{
    HistoryAtom, ProjectedLlmRequestHistory, ProviderContextReference, TrajectoryHistoryProjection,
};
...
#[derive(Clone, Debug)]
pub(super) struct TrajectoryClassifierConfig {
...
pub(super) struct TrajectoryAssignment {
    pub(super) action_id: String,
    pub(super) trajectory_id: String,
    pub(super) parent_action_id: Option<String>,
    pub(super) forked_from_action_id: Option<String>,
    pub(super) position: u32,
    pub(super) transition: LlmTrajectoryTransition,
    pub(super) start_reason: LlmTrajectoryStartReason,
    pub(super) inference_version: u32,
}

pub(super) enum TrajectoryClassification {
...
pub(super) struct TrajectoryClassifier {
...
impl TrajectoryClassifier {
    pub(super) fn new(config: TrajectoryClassifierConfig) -> Self {
        Self {
            config,
            scopes: HashMap::new(),
            resolved: Vec::new(),
    ...
    pub(super) fn classify_failure(&mut self, action_id: String) -> TrajectoryAssignment {
        singleton_assignment(action_id, LlmTrajectoryStartReason::ClassifierFailure)
    }

    pub(super) fn take_resolved(&mut self) -> Vec<TrajectoryAssignment> {
        std::mem::take(&mut self.resolved)
    }

    pub(super) fn register_provider_response(
        &mut self,
        trace_id: TraceId,
        process: ProcessIdentity,
        classifier_id: String,
    ...
    pub(super) fn finalize_trace(&mut self, trace_id: TraceId) -> Vec<TrajectoryAssignment> {
    ...
    fn classify_supported(
        ...
        let assignment = match parent {
            Some(parent) if !parent.has_continuation => {
                let parent_action_id = parent.action_id.clone();
                let trajectory_id = parent.trajectory_id.clone();
                let position = parent.position.saturating_add(1);
                ...
                if let Some(trajectory) = self.trajectories.get_mut(&trajectory_id) {
...
impl TrajectoryAssignment {
    pub(super) fn lineage(&self, trace_id: TraceId) -> LlmRequestLineageWrite {
