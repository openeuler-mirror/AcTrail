...
- `rustls` executable symbols: uses the target ELF symbol table and local demangling for `rustls::common_state::CommonState::buffer_plaintext` and `rustls::common_state::CommonState::take_received_plaintext`, including Rust v0 `<CommonState>::method` symbols with crate disambiguators removed through the demangler's stable alternate format, then emits the runtime `rustls_buffer_plaintext` and `rustls_take_received_plaintext` symbol-map keys.
...
Rustls versions and compiler output shapes are modeled as independent candidates. A candidate is selected only when both plaintext directions match uniquely; adding a newer candidate does not replace or widen earlier signatures. The aarch64 rustls 0.23.42 ThinLTO candidate was collected from the stripped xiaoO release binary and verified as unique:
