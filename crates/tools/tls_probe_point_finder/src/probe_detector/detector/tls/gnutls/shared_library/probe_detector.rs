...
use crate::probe_detector::detector::tls::gnutls::{RESOLVER, SYMBOLS};

use super::GnuTlsSharedLibraryProbeDetectorConfig;

pub(crate) struct GnuTlsSharedLibraryProbeDetector {
    path: DetectorPath,
}

impl GnuTlsSharedLibraryProbeDetector {
    pub(crate) fn try_new(
        config: GnuTlsSharedLibraryProbeDetectorConfig,
    ) -> Result<Self, DetectorConfigError> {
        config.validate()?;
        Ok(Self {
            path: DetectorPath::root(DetectorId::new("tls"))
                .child(DetectorId::new("gnutls"))
                .child(DetectorId::new("shared-library")),
        })
    }

    pub(crate) fn candidates(&self, target: &Path, libraries: &[PathBuf]) -> Vec<PathBuf> {
        Self::unique_candidates(target, libraries, "libgnutls")
    }
...
impl ProbeDetector for GnuTlsSharedLibraryProbeDetector {
    ...
    fn detect(&self, context: &ProbeContext<'_>) -> Result<DetectionOutcome, DetectionError> {
        if context.probe.source != ProbeSource::SharedLibrary {
            return Ok(DetectionOutcome::Inapplicable(
                DetectionEvidence::new(self.path.clone(), context.target.architecture.clone())
                    .rejected("GnuTLS resolver requires a shared-library context"),
            ));
        }
        let Some(symbols) = self
            ...
            return Ok(DetectionOutcome::NoMatch(
                DetectionEvidence::new(self.path.clone(), context.target.architecture.clone())
                    .rejected("GnuTLS plaintext symbols were not found"),
            ));
        };
        let mut evidence =
