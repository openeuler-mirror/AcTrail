...
use super::CommonStatePair5264X22ProbeDetectorConfig;
use super::verified_targets::verified_targets;

// CommonState plaintext entries using x22 for the payload tag temporary.
const BUFFER_PATTERN: &[u8] = &[
    0xff, 0x83, 0x01, 0xd1, 0xfd, 0x7b, 0x02, 0xa9, 0xf8, 0x5f, 0x03, 0xa9, 0xf6, 0x57, 0x04, 0xa9,
    0xf4, 0x4f, 0x05, 0xa9, 0xfd, 0x83, 0x00, 0x91, 0x16, 0x84, 0x41, 0xf9, 0x08, 0x00, 0xf0, 0xd2,
...
const TAKE_PATTERN: &[u8] = &[
    ...
    0xfd, 0x03, 0x00, 0x91, 0x36, 0x50, 0x40, 0xa9, 0x09, 0x00, 0xf0, 0xd2, 0x33, 0x08, 0x40, 0xf9,
    0xf5, 0x03, 0x00, 0xaa, 0x08, 0x04, 0x80, 0x52, 0x08, 0xb8, 0x0c, 0x39, 0xdf, 0x02, 0x09, 0xeb,
    0xa1, 0x00, 0x00, 0x54, 0x73, 0x01, 0xf8, 0xb6, 0xe0, 0x03, 0x1f, 0xaa, 0xe1, 0x03, 0x13, 0xaa,
];

pub(crate) struct CommonStatePair5264X22ProbeDetector;

impl CommonStatePair5264X22ProbeDetector {
    pub(crate) fn try_new(
        config: CommonStatePair5264X22ProbeDetectorConfig,
    ...
    pub(crate) fn into_pattern_pair(self) -> PatternPairProbeDetector {
        PatternPairProbeDetector::new(
            "aarch64-rustls-common-state-pair-52-64-x22",
            "aarch64",
            [
                StaticPatternSpec {
                    pattern_id: "aarch64-rustls-common-state-buffer-plaintext-entry-52-x22",
                    symbol: RUNTIME_BUFFER_PLAINTEXT_SYMBOL,
                    bytes: BUFFER_PATTERN,
                },
                StaticPatternSpec {
                    pattern_id: "aarch64-rustls-common-state-take-received-plaintext-entry-64-x22",
