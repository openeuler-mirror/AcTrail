...
use super::OpenSslSharedLibrarySymbolProbeDetectorConfig;

pub(crate) struct OpenSslSharedLibrarySymbolProbeDetector {
    path: DetectorPath,
    expected_arch: Arch,
}

impl OpenSslSharedLibrarySymbolProbeDetector {
    pub(crate) fn try_new(
        config: OpenSslSharedLibrarySymbolProbeDetectorConfig,
        expected_arch: Arch,
...
impl ProbeDetector for OpenSslSharedLibrarySymbolProbeDetector {
    ...
    fn detect(&self, context: &ProbeContext<'_>) -> Result<DetectionOutcome, DetectionError> {
        if context.probe.image.arch() != self.expected_arch {
            return Ok(DetectionOutcome::Inapplicable(
                DetectionEvidence::new(self.path.clone(), context.target.architecture.clone())
                    .rejected(format!(
                        "{} OpenSSL shared-library symbol detector received another architecture",
        ...
        if context.probe.source != ProbeSource::SharedLibrary {
            return Ok(DetectionOutcome::Inapplicable(
                DetectionEvidence::new(self.path.clone(), context.target.architecture.clone())
                    .rejected("OpenSSL symbol detector requires a shared-library context"),
            ));
        }
        let Some(symbols) = self
            ...
            return Ok(DetectionOutcome::NoMatch(
                DetectionEvidence::new(self.path.clone(), context.target.architecture.clone())
                    .rejected("OpenSSL shared-library plaintext symbols were not found"),
            ));
        };
        let mut evidence =
