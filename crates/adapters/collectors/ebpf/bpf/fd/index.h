...
static __noinline int fd_release(
    ...
    if ((state.category == ACTRAIL_FD_CATEGORY_NET
        || state.category == ACTRAIL_FD_CATEGORY_IPC_UNIX_SOCKET)
        && event_trace_id) {
        struct actrail_socket_release_event *event =
        ...
        if (event) {
            ...
            init_current_event_header(
                &event->header,
                ACTRAIL_SOCKET_FD_RELEASE,
                sizeof(*event),
                event_trace_id
            );
            event->fd = fd;
            event->fd_object_generation = state.generation;
            actrail_event_submit(ctx, event);
        }
    }
    if (state.category == ACTRAIL_FD_CATEGORY_NET && remaining == 0 && event_trace_id) {
        struct actrail_network_event *event = actrail_event_reserve(sizeof(*event));
        ...
        if (event) {
            ...
            event->fd = fd;
            event->syscall_family = ACTRAIL_SYSCALL_FAMILY_SOCKET;
            event->fd_object_generation = state.generation;
            event->endpoint_role = ACTRAIL_ENDPOINT_REMOTE;
            event->endpoint = object.remote;
            actrail_event_submit(ctx, event);
        }
    }
    return 1;
