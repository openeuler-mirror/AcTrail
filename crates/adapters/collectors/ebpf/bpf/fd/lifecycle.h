...
static __always_inline int fd_open_enter(
    ...
    if (!operation_key || !(fd_category_enabled(ACTRAIL_FD_CATEGORY_FILE)
        || fd_category_enabled(ACTRAIL_FD_CATEGORY_DIRECTORY)
        || fd_category_enabled(ACTRAIL_FD_CATEGORY_IPC_FIFO)
        || fd_category_enabled(ACTRAIL_FD_CATEGORY_IPC_UNIX_SOCKET))) {
        return 0;
    }
    trace_id = lookup_current_detailed_trace(&tgid, &tid, &lookup_flags);
    if (!tgid || !trace_id) {
...
static __always_inline int fd_close_range_dispatch_enter(
    ...
    if (!operation_key || !fd_tracking_enabled()
        || ((__u32)ctx->args[2] & ACTRAIL_FD_CLOSE_RANGE_CLOEXEC)) {
        return 0;
    }
    trace_id = lookup_current_detailed_trace(&tgid, &tid, &lookup_flags);
    if (!tgid || !trace_id) {
...
static __always_inline int fd_close_dispatch_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    if (op.mode == ACTRAIL_FD_CLOSE_ONE) {
        ...
        if (state && (!file_identity || state->file_identity != file_identity)) {
...
static __always_inline int fd_dup_enter(
    ...
    if (!source && !(mode == ACTRAIL_FD_DUP_TARGET_FD && fd_lookup(pid, target_fd))) {
        return 0;
    }
    if (source) {
        op.source = *source;
        op.source_file_identity = op.source.file_identity;
    ...
    op.mode = mode;
    if (source && !(mode == ACTRAIL_FD_DUP_TARGET_FD && op.target_fd == source_fd)) {
        if (!fd_object_ref_reserve(pid, source_fd, &op.source, op.trace_id, ctx)) {
            return 0;
        }
        op.reference_reserved = op.source.category == ACTRAIL_FD_CATEGORY_NET;
    }
...
static __always_inline int fd_dup_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    if (source.category == ACTRAIL_FD_CATEGORY_NONE) {
        goto reconcile;
    }
    if (source_identity != target_identity) {
        ...
        candidate = *current;
        if (candidate.file_identity != target_identity) {
            goto reconcile;
        }
        ...
        reservation_held = source.category == ACTRAIL_FD_CATEGORY_NET;
    }
    target_identity = fd_kernel_file_identity(target_fd);
    if (target_identity == ACTRAIL_FD_FILE_IDENTITY_READ_FAILED) {
        goto rollback;
    ...
    if (!fd_install_state(op.pid, target_fd, &source)) {
        goto rollback;
    }
    reservation_held = 0;
    target_identity = fd_kernel_file_identity(target_fd);
...
reconcile:
    ...
    if (current) {
        generation = current->generation;
        if (!target_identity || current->file_identity != target_identity) {
            fd_release(
                op.pid,
                target_fd,
                generation,
...
static __always_inline __u32 fd_ioctl_flag_enter(struct trace_event_raw_sys_enter *ctx) {
    ...
    __u64 operation_key = current_kernel_pid_tgid();
    struct actrail_pending_fd_flag_op op = { .context_kind = context_kind };
    if (!operation_key || !process_observation_is_detailed(operation_key >> 32)) {
        return ACTRAIL_FD_FLAG_CONTEXT_NONE;
    }
    return bpf_map_update_elem(&pending_fd_flag_ops, &operation_key, &op, BPF_ANY) == 0
...
static __always_inline __u32 fd_flag_exit(struct trace_event_raw_sys_exit *ctx) {
    ...
    context_kind = pending->context_kind;
