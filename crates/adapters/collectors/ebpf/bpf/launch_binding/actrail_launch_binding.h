...
static __always_inline __u64 actrail_launch_binding_promote_current(
    ...
    if (!current_host_tgid || !pending_count || *pending_count == 0 ||
    ...
    if (!actrail_launch_binding_install_suppressed_fds(
        ...
        cleanup_suppressed_fds_for_process(current_host_tgid, generation);
        bpf_map_delete_elem(&process_start_times, &current_host_tgid);
        actrail_launch_binding_delete_invalid(ctx, &lookup, pending_count);
        return 0;
    }
    if (bpf_map_update_elem(
    ...
    if (deleted == ACTRAIL_LAUNCH_BINDING_DELETE_FAILED) {
        ...
        bpf_map_delete_elem(&tracked_traces, &current_host_tgid);
        cleanup_suppressed_fds_for_process(current_host_tgid, generation);
        bpf_map_delete_elem(&process_start_times, &current_host_tgid);
        return 0;
    }
