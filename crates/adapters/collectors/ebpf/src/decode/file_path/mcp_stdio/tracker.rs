...
impl McpStdioTracker {
    pub(in crate::decode::file_path) fn new(
        ...
        Self {
            enabled: mcp_stdio_projection_enabled,
            config,
            traces: BTreeMap::new(),
    ...
    pub(in crate::decode::file_path) fn enabled(&self) -> bool {
        self.enabled
    }
    ...
    pub(in crate::decode::file_path) fn record_pair(
        ...
        let projection_enabled = self.enabled;
    ...
    pub(in crate::decode::file_path) fn invalidate_fd_binding(
        &mut self,
        key: &ProcessFileKey,
        fd: u32,
        observed_ktime_ns: u64,
    ...
    pub(in crate::decode::file_path) fn mcp_stdio_diagnostics(&self) -> Vec<(&'static str, u64)> {
        if !self.enabled {
            return Vec::new();
        }
