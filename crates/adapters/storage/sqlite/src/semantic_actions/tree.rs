...
use crate::records::{decode_map, encode_time};
...
impl SqliteStorage {
    ...
    pub fn semantic_action_command_fallback_children(
        ...
        let start_time = encode_time(command.start_time);
        let mut values = vec![
            Value::Integer(i64::try_from(trace_id.get()).map_err(|error| {
            ...
            Value::Integer(i64::try_from(command.process.start_time_ticks).map_err(
                |error| {
                    SemanticActionStoreError::new(
