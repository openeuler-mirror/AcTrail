...
use semantic_action::{SemanticActionLinkRole, SemanticActionStoreError, attr_keys as attrs};

use crate::semantic_actions::codebook::sqlite::{
    decode_link_role, link_role_code, link_role_code_from_str,
};
...
pub(super) fn display_child_counts(
    ...
    for row in rows {
        ...
        if action_valid_code == 1 && link_valid == 1 && !role_conflicted {
            children_by_parent
